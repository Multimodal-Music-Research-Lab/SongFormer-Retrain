...
def load_optional_lyrics_embedding(audio_path, lyrics_embedding_dir, lyrics_input_dim):
    ...
    if lyrics_embedding.shape[0] != lyrics_input_dim:
        raise ValueError(
            f"Lyrics embedding dim mismatch: {lyrics_path}, "
            f"got {lyrics_embedding.shape[0]}, expected {lyrics_input_dim}"
        )

    return lyrics_embedding, 1.0
    ...
    with torch.no_grad():
        while True:
            ...
            try:
                ...
                if use_lyrics:
                    lyrics_np, has_lyrics_val = load_optional_lyrics_embedding(
                        audio_path=item,
                ...
                while True:
                    ...
                    msa_info, chunk_logits = model.infer(
                        ...
                        # =========================
                        # [ADDED FOR LYRICS]
                        # Optional song-level lyrics conditioning
                        # =========================
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional directory containing per-song lyrics .npy. "
            "Expected filename: <audio_stem>.npy ; "
            "if not found, inference falls back to zero-vector lyrics + has_lyrics=0."
        ),
    )
