...
RUN=/mnt/ssd/hbli/songformer/runs/hx_train_sxsinger_alma_fusion_v7
CKPT=${1:-${RUN}/results/train_output_42/model.ckpt-41600.pt}
CKPT_TAG=$(basename "${CKPT}")
CKPT_TAG=${CKPT_TAG%.pt}
CFG=${REPO}/runs/hx_train_sxsinger_alma_fusion_v7/configs/SongFormer.yaml
...
run_full_eval() {
  ...
  python ${REPO}/runs/hx_train_sxsinger_alma_fusion_v7/scripts/infer.py \
    -i "${scp}" \
    -o "${pred_dir}" \
    -gn 1 -tn 1 \
    --model SongFormer \
...
run_full_eval \
  ...
  /mnt/ssd/hbli/datasets/songformer/songformbench/data/soulxsinger_lyrics/HarmonixSet
...
run_full_eval \
  ...
  /mnt/ssd/hbli/datasets/songformer/songformbench/data/soulxsinger_lyrics/CN
