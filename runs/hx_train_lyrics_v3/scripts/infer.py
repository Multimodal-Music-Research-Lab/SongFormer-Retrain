...
# If not found, return one zero unit for both line/stanza + has_lyrics=0
# =========================
def get_zero_lyrics_units(lyrics_input_dim):
    zero_unit = np.zeros((1, lyrics_input_dim), dtype=np.float32)
...
def validate_lyrics_units(lyrics_units, feat_dim, source_path, field_name):
    ...
    if lyrics_units.shape[0] == 0:
        raise ValueError(f"{field_name} is empty in {source_path}")
    return lyrics_units


def load_optional_lyrics_units(audio_path, lyrics_embedding_dir, lyrics_input_dim):
    ...
    if lyrics_embedding_dir is None or str(lyrics_embedding_dir).strip() == "":
        zero_line, zero_stanza = get_zero_lyrics_units(lyrics_input_dim)
        return zero_line, zero_stanza, 0.0
    ...
    if lyrics_npz_path.exists():
        ...
        return line_embs.astype(np.float32), stanza_embs.astype(np.float32), 1.0
    ...
    if lyrics_npy_path.exists():
        ...
        return single_unit, single_unit.copy(), 1.0
    ...
    with torch.no_grad():
        while True:
            ...
            try:
                ...
                # =========================
                # [ADDED FOR LYRICS]
                # Load raw line/stanza units once for the whole song.
                # They are shared by all chunks of this song.
                # =========================
                ...
                if use_lyrics:
                    line_np, stanza_np, has_lyrics_val = load_optional_lyrics_units(
                        audio_path=item,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional directory containing per-song lyrics embeddings. "
            "Preferred filename: <audio_stem>.npz with fields {line_embs, stanza_embs}; "
            "fallback: <audio_stem>.npy. "
            "If not found, inference falls back to one zero line/stanza unit + has_lyrics=0."
        ),
    )
