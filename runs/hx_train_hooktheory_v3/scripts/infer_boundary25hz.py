    ...
    with torch.no_grad():
        while True:
            ...
            try:
                ...
                function_total_frames = math.ceil(
                    total_len * AFTER_DOWNSAMPLING_FRAME_RATES
                )
                boundary_frame_rate = (
                    BEFORE_DOWNSAMPLING_FRAME_RATES
                    if args.boundary_mode == "highres"
                    else AFTER_DOWNSAMPLING_FRAME_RATES
                )
                boundary_total_frames = math.ceil(
                    total_len * boundary_frame_rate
                )

                logits = {
                    "function_logits": np.zeros(
                        [function_total_frames, num_classes]
                    ),
                    "boundary_logits": np.zeros([boundary_total_frames]),
                }
                logits_num = {
                    "function_logits": np.zeros(
                        [function_total_frames, num_classes]
                ...
                while True:
                    ...
                    function_start_frame = int(
                        i * AFTER_DOWNSAMPLING_FRAME_RATES
                    )
                    function_end_frame = function_start_frame + min(
                        math.ceil(hop_size * AFTER_DOWNSAMPLING_FRAME_RATES),
                        chunk_logits["function_logits"][0].shape[0],
                    )
                    boundary_key = (
                        "boundary_logits_25hz"
                        if args.boundary_mode == "highres"
                        else "boundary_logits"
                    )
                    boundary_start_frame = int(i * boundary_frame_rate)
                    boundary_end_frame = boundary_start_frame + min(
                        math.ceil(hop_size * boundary_frame_rate),
                        chunk_logits[boundary_key][0].shape[0],
                    )

                    logits["function_logits"][
                        function_start_frame:function_end_frame, :
                    ] += (
                        chunk_logits["function_logits"][0, : function_end_frame - function_start_frame]
                        .detach()
                        .cpu()
                        .numpy()
                    )
                    logits["boundary_logits"][
                        boundary_start_frame:boundary_end_frame
                    ] += (
                        chunk_logits[
                            boundary_key
                        ][0, : boundary_end_frame - boundary_start_frame]
                        .detach()
                        .cpu()
                        .numpy()
                    )
                    logits_num["function_logits"][
                        function_start_frame:function_end_frame, :
                    ] += 1
                    logits_num["boundary_logits"][
                        boundary_start_frame:boundary_end_frame
                    ] += 1
                    function_lens = max(function_lens, function_end_frame)
                    boundary_lens = max(boundary_lens, boundary_end_frame)

                    i += hop_size

                logits["function_logits"][:function_lens] /= logits_num[
                    "function_logits"
                ][:function_lens]
                logits["boundary_logits"][:boundary_lens] /= logits_num[
                    "boundary_logits"
                ][:boundary_lens]

                logits["function_logits"] = torch.from_numpy(
                    logits["function_logits"][:function_lens]
                ).unsqueeze(0)
                logits["boundary_logits"] = torch.from_numpy(
                    logits["boundary_logits"][:boundary_lens]
                ).unsqueeze(0)

                msa_infer_output = postprocess_functional_structure(
                    logits,
                    hp,
                    boundary_frame_rates=boundary_frame_rate,
                    function_frame_rates=AFTER_DOWNSAMPLING_FRAME_RATES,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="highres",
        help="Use the original 8.33 Hz or the independent 25 Hz boundary branch.",
    )
    parser.add_argument(
