...
"""
Build a jsonl file compatible with HookTheoryV1Adapter, and record bad .sec files.

Input:
  1) .scp file: one absolute mp3 path per line
  2) section_dir: directory containing .sec files (matched by <mp3_stem>_measure.sec)
     Each .sec line format:  <start_seconds> <end_seconds> <Label>

Output jsonl (one segment per line), with fields used by HookTheoryV1Adapter:
  - ori_audio_path: str
  - segment_start: float
  - segment_end: float
  - label: List[str]  (already mapped to SongFormer 8 labels)

Also outputs:
  - bad_sec_list: a text file listing sec file paths that failed parsing
"""
...
MAPPING = {
    ...
    "solo": "inst",
    "verse": "verse",
    # no silence in .sec now, kept for safety
    "silence": "silence",
}
...
def find_sec_for_audio(audio_path: Path, section_dir: Path, sec_ext: str) -> Optional[Path]:
    """
    Your naming rule:
      section_dir/<mp3_stem>_measure.sec

    NOTE: sec_ext should be like ".sec"
    """
    sec_path = section_dir / f"{audio_path.stem}_measure{sec_ext}"
...
def build_jsonl(
    ...
    with out_jsonl.open("w", encoding="utf-8") as w:
        for audio_path in audio_paths:
            ...
            sec_path = find_sec_for_audio(audio_path, section_dir, sec_ext)
            if sec_path is None:
                sec_missing += 1
                if require_sec:
                    continue
            ...
            try:
                segments = parse_sec_file(sec_path)
            except Exception:
                bad_sec += 1
    ...
    print(f"Missing .sec skipped: {sec_missing} (require_sec={require_sec})")
    print(f"Bad .sec skipped: {bad_sec}")
...
def main():
    ...
    ap.add_argument("--allow_missing_sec", action="store_true", help="Skip entries without matching .sec")
    ap.add_argument(
        "--bad_sec_list",
        type=str,
        default=None,
