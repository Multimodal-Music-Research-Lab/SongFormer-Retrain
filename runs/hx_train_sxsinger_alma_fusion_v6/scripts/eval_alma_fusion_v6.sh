...
RUN=/mnt/ssd/hbli/songformer/runs/hx_train_sxsinger_alma_fusion_v6
CKPT=${1:-${RUN}/results/train_output_42/model.ckpt-48000.pt}
CKPT_TAG=$(basename "${CKPT}")
CKPT_TAG=${CKPT_TAG%.pt}
CFG=${REPO}/runs/hx_train_sxsinger_alma_fusion_v6/configs/SongFormer.yaml
...
python ${REPO}/runs/hx_train_sxsinger_alma_fusion_v6/scripts/infer.py \
  -i ${BENCH_SCP} \
  -o ${PRED_DIR} \
  -gn 1 -tn 1 \
  --model SongFormer \
